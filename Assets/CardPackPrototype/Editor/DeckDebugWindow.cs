...
namespace CardOpen.Editor
{
    public sealed class DeckDebugWindow : EditorWindow
    {
        ...
        [MenuItem("CardOpen/Debug/덱에 잎 즉시 추가")]
        private static void Open()
        {
            GetWindow<DeckDebugWindow>("덱 잎 추가");
        }

        private void OnGUI()
        {
            EditorGUILayout.LabelField("덱에 잎 즉시 추가", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox("플레이 중인 CardOpen 게임의 덱에 잎을 추가합니다. 기존 병합, 장착, 조합 규칙을 적용합니다.", MessageType.Info);
            ...
            selectedCard = (CardData)EditorGUILayout.ObjectField("잎", selectedCard,
                typeof(CardData), false);
            cardNumber = EditorGUILayout.IntSlider("잎 숫자", cardNumber, 1, 6);
            cardColor = (CardColor)EditorGUILayout.EnumPopup("잎 색상", cardColor);
            ...
            using (new EditorGUI.DisabledScope(selectedCard == null))
            {
                if (GUILayout.Button("덱에 즉시 추가", GUILayout.Height(34f)))
                {
                    if (game.DebugAddCardToDeck(selectedCard, cardNumber, cardColor))
                        feedback = selectedCard.GetLocalizedName(false) + " 잎을 덱에 추가했습니다.";
                    else
                        feedback = "덱이 가득 찼거나 잎을 추가할 수 없습니다.";
                }
            }
